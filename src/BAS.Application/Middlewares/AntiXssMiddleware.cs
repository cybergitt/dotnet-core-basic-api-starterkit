...
{
    public class AntiXssMiddleware(ILogger<AntiXssMiddleware> logger) : IMiddleware
    {
        private readonly ILogger<AntiXssMiddleware> _logger = logger;
        ...
        private void ResponseErrorWithLog(HttpContext context)
        {
            const string errorMessage = "XSS injection detected from input.";
            _logger.LogError("Stacktrace of error: XSS injection detected from input. {errorMessage}", errorMessage);
            throw new BadHttpRequestException(ApiMessages.BadRequest + ": " + errorMessage);
        }
